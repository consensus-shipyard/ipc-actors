...
use std::str::FromStr;
use primitives::TCid;
use ipc_gateway::{Checkpoint, DEFAULT_CHECKPOINT_PERIOD, get_bottomup_msg, IPCAddress, State, StorableMsg, SubnetID};
...
fn release_stake() {
    ...
    h.release_stake(&mut rt, &shid, &TokenAmount::from_atto(5u64.pow(17)), ExitCode::OK).unwrap();
    let subnet = h.get_subnet(&rt, &shid).unwrap();
    ...
    h.release_stake(&mut rt, &shid, &TokenAmount::from_atto(5u64.pow(17)), ExitCode::USR_ILLEGAL_STATE)
        .unwrap();
}
...
fn checkpoint_crossmsgs() {
    ...
    add_msg_meta(
        &mut ch,
        &shid,
        &SubnetID::from_str("/root/f12/f1").unwrap(),
    ...
    add_msg_meta(
        &mut ch,
        &shid,
        &SubnetID::from_str("/root/f12/f2").unwrap(),
    ...
    h.commit_child_check(&mut rt, &shid, &ch, ExitCode::OK, 2 * TokenAmount::from_atto(5_u64.pow(18)))
        .unwrap();
    let st: State = rt.get_state();
    let commit = st.get_window_checkpoint(rt.store(), epoch).unwrap();
    assert_eq!(commit.epoch(), DEFAULT_CHECKPOINT_PERIOD);
...
//     h.send_cross(&mut rt, &from, &shid, &to, sub, ExitCode::OK, value.clone(), 1, &value).unwrap();
//     let sub = SubnetID::from_str("/root/f0101/f0101").unwrap();
...
//     let ff =
//         IPCAddress::new(&SubnetID::from_str("/root/f01/f012").unwrap(), &from).unwrap();
//     let tt = IPCAddress::new(&sub1, &to).unwrap();
...
//     let tt = IPCAddress::new(&SubnetID::from_str("/root/f0101/f0102/f011").unwrap(), &to)
//         .unwrap();
//     h.apply_cross_msg(&mut rt, &ff, &tt, value.clone(), 0, 2, ExitCode::OK, false).unwrap();
//     let ff = IPCAddress::new(&sub2, &from).unwrap();
//     let tt = IPCAddress::new(&SubnetID::from_str("/root/f0101/f0101/f011").unwrap(), &to)
//         .unwrap();
//     h.apply_cross_msg(&mut rt, &ff, &tt, value.clone(), 1, 3, ExitCode::OK, false).unwrap();
//     // directed to current network
//     let ff = IPCAddress::new(&SubnetID::from_str("/root/f0101/f0102/f011").unwrap(), &from).unwrap();
//     let tt = IPCAddress::new(&shid, &to).unwrap();
...
fn gen_exec_msgs(addr: IPCAddress) -> Vec<StorableMsg> {
    return vec![
        StorableMsg {
            from: addr.clone(),
            to: addr.clone(),
